...
def get_calculated_cost(request: HttpRequest) -> HttpResponse:
    ...
    cost = helpers.calculate_cost_of_one_paper(
        ...
        models.Cost.objects.filter(paper=models.Paper.objects.get(
            id=paper_id)).order_by('day').values_list('cost', flat=True),
        models.Cost.objects.filter(paper=models.Paper.objects.get(
            id=paper_id)).order_by('day').values_list('delivery', flat=True)
    )
